...
export const CelestialArc: React.FC<CelestialArcProps> = ({
  ...
  const transformedX = useTransform(mouseX, (val) => (val / window.innerWidth - 0.5) * 2 * parallaxStrength)
  const transformedY = useTransform(mouseY, (val) => (val / window.innerHeight - 0.5) * 2 * parallaxStrength)
